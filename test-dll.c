...
{  
   ...
   for(  i=2425320; i>0; i=i-3){
      insertDLL(items,sizeDLL(items),newINTEGER(i));
   }
   for(  b=-23; b<338; b=b+4){
      insertDLL(items,3,newINTEGER(b));}
   for(  c=0; c<90; c++){
      INTEGER *x = removeDLL(items,1);
      freeINTEGER(x);
   }
   int x = sizeDLL(items)-1;

   for( d=x; d>1000 ;d--){
      INTEGER *y = removeDLL(items,d);
      freeINTEGER(y);
   }

   int y = sizeDLL(items)-5;
   for (i=y/2; i>1;i--){
      INTEGER *z = removeDLL(items,i);
      freeINTEGER(z);
   }
   int n = sizeDLL(items)-4;
   for(i=n;i>0;i--){
      INTEGER *g = removeDLL(items,i);
      freeINTEGER(g);
   }
   displayDLL(items,stdout);
   DLL *items2 = newDLL(displayINTEGER,freeINTEGER);
   for (i=0;i<100000; i=i+3){
      insertDLL(items2,0,newINTEGER(i));
   }
   unionDLL(items,items2);
   displayINTEGER(getDLL(items,21),stdout);
   printf("\n");
      freeDLL(items2);
   freeDLL(items);
      return 0;
